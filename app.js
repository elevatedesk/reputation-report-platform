...
function render(){
  ...
  $("#mediaList").innerHTML=(report.mediaMentions || []).slice(0,6).map(x=>detail(x.title,`${x.source} · ${x.date}`,x.note)).join("") || "<p class='muted'>No third-party media mentions found yet.</p>";
...
function counts(report){
  ...
  return {
    google:Math.max((report.researchResults?.webResults||[]).length,assets.length),
    news:Math.max((report.researchResults?.newsResults||[]).length,0),
    reach:(report.researchResults?.newsResults||[]).length ? "Pending" : "0",
    speaking:Math.max(assets.filter(a=>a.type==="Speaking Engagement").length,(report.researchResults?.speakingResults||[]).length),
    awards:Math.max(assets.filter(a=>a.type==="Award / Recognition").length,(report.researchResults?.awardResults||[]).length),
...
function drawerItem(type,title,meta,note){
  const clientKey = activeClientId || "global";
  hidden[clientKey] = hidden[clientKey] || [];
  const key=`${type}|${title}|${meta}`;
  if(hidden[clientKey].includes(key)) return "";
...
function metricData(type){
  ...
  const assets = activeAssets();
  const data = {
    google:{title:"Google Results",subtitle:"Live web results tied to name and LinkedIn.",items:[...(rr.webResults||[]).map(x=>drawerItem("google",x.title,x.source,x.snippet||x.link))]},
    news:{title:"News + Media Mentions",subtitle:"Third-party articles, profiles, spotlights, expert commentary, and publication features.",items:[...(rr.newsResults||[]).map(x=>drawerItem("news",x.title,x.source,x.snippet||x.link)),...assets.filter(a=>["Media Mention","Podcast / Interview","Publication / Article"].includes(a.type)).map(a=>drawerItem("news",a.title,`${a.source} · ${a.date}`,`${a.role} · ${a.notes}`))]},
    ...
    speaking:{title:"Speaking Intelligence",subtitle:"Conferences, summits, forums, keynotes, panels, workshops, breakouts, webinars, podcasts, and university lectures.",items:[...(rr.speakingResults||[]).map(x=>drawerItem("speaking",x.title,x.source,x.snippet||x.link)),...assets.filter(a=>a.type==="Speaking Engagement").map(a=>drawerItem("speaking",a.title,`${a.role} · ${a.source} · ${a.date}`,a.notes))]},
    awards:{title:"Awards + Recognition",subtitle:"Third-party awards, rankings, best-of lists, top lists, trailblazers, honorees, and formal recognition.",items:[...(rr.awardResults||[]).map(x=>drawerItem("awards",x.title,x.source,x.snippet||x.link)),...assets.filter(a=>a.type==="Award / Recognition").map(a=>drawerItem("awards",a.title,`${a.source} · ${a.date}`,`${a.role} · ${a.notes}`))]},
...
function renderPages(){
  ...
  $("#rep360").innerHTML = basicPage("Reputation 360™",currentReport().executiveSummary || "Generate a report for this client.");
...
function reportsPage(){
  const c = activeClient();
  if(!c) return `<div class="card panel"><h4>Reports</h4><p class="muted">No client selected.</p></div>`;
  return `<div class="card panel"><h4>${mode==="admin" ? "Reports for " + c.name : "My Reports"}</h4>
...
function intakePage(){
  const c = activeClient();
  if(!c) return `<div class="card panel"><h4>Client Intake</h4><p class="muted">Select a client first.</p></div>`;
...
function assetManagerPage(){
  const c = activeClient();
  if(!c) return `<div class="card panel"><h4>Assets</h4><p class="muted">Select a client first.</p></div>`;
  ...
  return `<div class="card panel"><h4>${mode==="admin"?"Manual Asset Manager":"My Submitted Assets"}: ${c.name}</h4><p class="muted">${mode==="admin"?"Assets stay tied to this client only.":"View assets submitted or included in your report."}</p>${form}<h4 style="margin-top:24px">Assets</h4><div id="assetList">${assetList()}</div></div>`;
}
function assetList(){
  const assets = activeAssets();
...
function speakingPage(){ const r=currentReport(); return `<div class="card panel"><h4>Speaking Intelligence</h4><p class="muted">Checks conferences, summits, forums, keynotes, panels, workshops, breakout sessions, webinars, podcasts, university lectures, and community events tied to the selected client.</p>${(r.speakingEngagements||[]).map(x=>detail(x.title,`${x.eventType} · ${x.role} · ${x.date}`,x.note)).join("") || "<p class='muted'>No speaking signals yet.</p>"}</div>`; }
...
function bindDynamic(){
  ...
  $$("[data-hide]").forEach(b=>b.onclick=()=>{hidden[activeClientId]=hidden[activeClientId]||[]; hidden[activeClientId].push(b.dataset.hide); save("rr_hidden",hidden); b.closest(".detail-item").remove(); toast("Removed irrelevant item.");});
